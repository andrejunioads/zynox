...
interface SelectTeamMemberModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (members: TeamMember[]) => void;
  availableMembers: Member[];
  currentProjectMembers: TeamMember[];
}

export const SelectTeamMemberModal = ({ 
  isOpen, 
  onClose, 
  onAdd, 
  availableMembers,
  currentProjectMembers 
}: SelectTeamMemberModalProps) => {
  ...
  const handleSubmit = () => {
    if (selectedMembers.length === 0) {
      alert('Selecione pelo menos um membro para adicionar ao projeto.');
      return;
    }
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[80vh] glass-card border-primary/20">
        <DialogHeader className="pb-6">
          ...
          <p className="text-slate-400 text-sm mt-2">
            Selecione os membros da equipe para adicionar ao projeto
          </p>
        </DialogHeader>
          ...
          {selectedMembers.length > 0 && (
            <div className="flex items-center gap-2 px-4 py-3 bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/20 rounded-xl">
              ...
              <div>
                ...
                <p className="text-green-400/70 text-xs">Pronto para adicionar ao projeto</p>
              </div>
            </div>
          )}
          ...
          <div className="max-h-96 overflow-y-auto space-y-3">
            {filteredMembers.length === 0 ? (
              ...
              filteredMembers.map((member) => {
                ...
                const isAlreadyInProject = currentProjectMembers.some(m => m.id === member.id);
                ...
                return (
                  <div
                    ...
                    onClick={() => !isAlreadyInProject && handleMemberToggle(member.id)}
                  >
                    {/* Selection indicator */}
                    <div className="flex-shrink-0">
                      <Checkbox
                        ...
                        onChange={() => !isAlreadyInProject && handleMemberToggle(member.id)}
          ...
          >
            {selectedMembers.length > 0 ? (
              <div className="flex items-center gap-2">
                ...
                Adicionar {selectedMembers.length} membro{selectedMembers.length > 1 ? 's' : ''}
              </div>
            ) : (
              'Selecione membros'
            )}
